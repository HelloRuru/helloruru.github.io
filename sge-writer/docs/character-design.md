...
|------|------|
| 尺寸 | **768×1344 px** |
| 格式 | PNG（**透明背景**，白背景用 `sharp unflatten` 去背） |
| 風格 | 日系動漫立繪（非 Q 版） |
| 表情 | 預設/樂/喜/怒/哀，每種最多 4 張變體 |
| 總數量 | **51 張表情 + 39 張場景**（見下表） |
...
| 原始素材名稱 | 對應表情 | 檔名前綴 |
|-------------|----------|----------|
| 基本 | default（預設） | `player-default-` |
| 打招呼 | joy（樂） | `player-joy-` |
| 害羞 | happy（喜） | `player-happy-` |
| 緊張 | angry（怒） | `player-angry-` |
| 困惑 | sad（哀） | `player-sad-` |
| 作筆記 | 場景圖 | `scene-player-notes-` |
